import { format } from 'path';
import * as path from 'path';
import { YtDlp } from 'ytdlp-nodejs';
import * as fs from 'fs';
import { withYouTubeLock } from './youtube-lock.js';

const ytdlp = new YtDlp();

const CHANNEL_ARCHIVE_FILE = path.join(
    process.cwd(),
    "data",
    "channel-download-archive.txt"
);

// helper function to prevent YouTube shorts videos from being downloaded
export function isYouTubeShortUrl(url) {
    try {
        const parsedUrl = new URL(url);
        const hostname = parsedUrl.hostname.toLowerCase().replace(/^www\./, "");

        return (hostname === "youtube.com" || hostname === "m.youtube.com")
            && /^\/shorts(?:\/|$)/i.test(parsedUrl.pathname);
    } catch {
        return false;
    }
}

export async function downloadNew(channel, folder) {
    // The channel root includes uploads from the Shorts tab. Restrict discovery
    // to the Videos tab so only long-form uploads are considered by yt-dlp.
    const url = "https://www.youtube.com/@" + channel + "/videos";
    const baseDir = "J:/Youtube/" + folder;

    try {
        const output = await ytdlp.downloadAsync(
        url,
        {
            format: {
                filter: 'mergevideo',
                quality: 'highest',
                format: 'mkv'
            },
            onProgress: (progress) => {
                console.log(channel + " video progress", progress.percentage);
            },
            output: path.join(
                baseDir,
                "Season %(upload_date>%Y)s",
                "%(title)s [%(id)s]",
                "%(title)s [%(id)s].%(ext)s"
            ),
            embedChapters: true,
            embedInfoJson: true,
            embedSubs: true,
            embedMetadata: true,
            embedThumbnail: true,
            writeThumbnail: true,
            sleepRequests: 2,
            sleepInterval: 10,
            maxSleepInterval: 30,
            cookies: "cookies-yt.txt",
            writeInfoJson: true,
            downloadArchive: CHANNEL_ARCHIVE_FILE,
            noWritePlaylistMetafiles: true,
            dateAfter: "today-5day",
            breakOnReject: true,
            verbose: true
        }
        );
        console.log('Channel update completed:', output);

    } catch (error) {
        if (error.message?.includes("code 101")) {
            console.log('Error: No new videos found for this channel.');
        } else {
            console.error('Error:', error);
        }
    } finally {
        try {
            await generateNFO(folder);
        } catch (error){
            console.error("Failed to generate NFO files:", error);
        }
        
    }
}

function findInfoJsonFiles(directory) {
    return fs.readdirSync(directory, { withFileTypes: true }).flatMap(entry => {
        const entryPath = path.join(directory, entry.name);

        if (entry.isDirectory()) {
            return findInfoJsonFiles(entryPath);
        }

        if (entry.isFile() && entry.name.endsWith(".info.json")) {
            return [entryPath];
        }

        return [];
    });
}

async function generateNFOsInDirectory(baseDir) {
    //now generate .nfo files for each video
    // Find all .info.json files in the channel and its video subfolders.
    const allJSONs = findInfoJsonFiles(baseDir);

    // Only generate NFOs for videos that don't already have one
    const missingNFOs = allJSONs.filter(jsonPath => {
        const baseName = jsonPath.replace(/\.info\.json$/, "");
        const nfoPath = baseName + ".nfo";
        return !fs.existsSync(nfoPath); 
    });

    if (missingNFOs.length === 0) {
        console.log("No new NFO files to generate.");
        return;
    }

    for (const jsonPath of missingNFOs) {
        try {

            const file = path.basename(jsonPath);
            const videoDirectory = path.dirname(jsonPath);
            const allFiles = fs.readdirSync(videoDirectory);
            const info = JSON.parse(fs.readFileSync(jsonPath, "utf8"));

            const title = info.title;
            const safeTitle = title.replace(/[<>:"/\\|?*]/g, "");
            const id = info.id;
            const description = info.description || "";

            console.log(title);

            //get the upload date to use as season and episode numbers
            let uploadDate = info.upload_date;

            const season = uploadDate.slice(0, 4); 
            const monthDay = uploadDate.slice(4, 8); 
            const episode = 1232 - Number(monthDay);

            if (!/^\d{8}$/.test(uploadDate)) {
                const now = new Date(); 
                uploadDate = now.toISOString().slice(0,10).replace(/-/g,""); 
            }

            const premiered = `${uploadDate.slice(0,4)}-${uploadDate.slice(4,6)}-${uploadDate.slice(6,8)}`;

            function escapeXml(str) { 
                return (str || "").replace(/[<>&'"]/g, c => ({
                    "<": "&lt;",
                    ">": "&gt;", 
                    "&": "&amp;", 
                    "'": "&apos;", 
                    '"': "&quot;" 
                }[c]));
            }

            // Find the actual video filename 
            const baseName = file.replace(".info.json", ""); 
            const videoFile = allFiles.find(f => 
                f.startsWith(baseName) && (f.endsWith(".mkv") || f.endsWith(".mp4") || f.endsWith(".webm")) 
            ); 
            
            if (!videoFile) { 
                console.log(`No matching video file found for ${file}, skipping.`); 
                continue; 
            }

            const nfoName = videoFile.replace(path.extname(videoFile), ".nfo");


            const nfo = ` 
                <episodedetails> 
                    <title>${escapeXml(title)}</title> 
                    <originaltitle>${escapeXml(title)}</originaltitle>
                    <season>${season}</season>
                    <episode>${episode}</episode> 
                    <plot>${escapeXml(description)}</plot> 
                    <premiered>${premiered}</premiered> 
                    <uniqueid type="youtube" default="true">${id}</uniqueid>
                </episodedetails> `
            .trim();

            fs.writeFileSync(path.join(videoDirectory, nfoName), nfo);
            console.log(`Created NFO for: ${nfoName}`);
            
        } catch (error) {
            console.error( `Failed to generate NFO from ${jsonPath}:`, error );
        }
    }
    console.log("All NFO files created successfully.");
}

export async function generateNFO(folder) {
    return generateNFOsInDirectory(path.join("J:/Youtube", folder));
}

async function performVideoDownload(url) {
    const outputDirectory = isYouTubeShortUrl(url)
        ? "J:/Youtube/Shorts/"
        : "J:/Youtube/Downloads/";

    const outputTemplate = path.join(
        outputDirectory,
        "%(title)s [%(id)s]",
        "%(title)s [%(id)s].%(ext)s"
    );

    try {
        const output = await ytdlp.downloadAsync(
        url,
        {
            format: {
                filter: 'mergevideo',
                quality: 'highest',
                format: 'mkv'
            },
            onProgress: (progress) => {
                console.log("video progress", progress.percentage);
            },
            output: outputTemplate,
            embedChapters: true,
            embedInfoJson: true,
            embedSubs: true,
            embedMetadata: true,
            embedThumbnail: true,
            writeThumbnail: true,
            sleepRequests: 2,
            sleepInterval: 10,
            maxSleepInterval: 30,
            cookies: "cookies-yt.txt",
            writeInfoJson: true,
            breakOnReject: true,
        }
        );
        console.log('Video download completed:', output);
    } catch (error) {
        console.error('Error:', error);
    } finally {
        try {
            await generateNFOsInDirectory(outputDirectory);
        } catch (error) {
            console.error("Failed to generate NFO files:", error);
        }
    }
}

export function downloadVideo(url) {
    return withYouTubeLock(
        "individual video download",
        () => performVideoDownload(url)
    );
}


async function performPlaylistDownload(url) {
    try {
        const output = await ytdlp.downloadAsync(
        url,
        {
            format: {
                filter: 'mergevideo',
                quality: 'highest',
                format: 'mkv'
            },
            onProgress: (progress) => {
                console.log("video progress", progress.percentage);
            },
            //needs to update the output to change based on the name of the playlist
            output: "J:/Youtube/Riley Rehl" + "/%(playlist_title)s/%(playlist_index)s - %(title)s.%(ext)s",
            embedChapters: true,
            embedInfoJson: true,
            embedSubs: true,
            embedMetadata: true,
            embedThumbnail: true,
            writeThumbnail: true,
            sleepRequests: 2,
            sleepInterval: 10,
            maxSleepInterval: 30,
            cookies: "cookies-yt.txt",
            writeInfoJson: true,
            breakOnReject: true,
        }
        );
        console.log('Video download completed:', output);
    } catch (error) {
        console.error('Error:', error);
    }
}

export function downloadPlaylists(url) {
    return withYouTubeLock(
        "legacy playlist download",
        () => performPlaylistDownload(url)
    );
}
